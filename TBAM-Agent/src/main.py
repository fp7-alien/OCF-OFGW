...
def main():
    ...
    class xmlrpc_registers:
        ...
        def setTCPProxy(self, controller):
            ...
            os.system("iptables -t nat -A POSTROUTING -j MASQUERADE -p tcp --dport %s" % (controller.split(":")[1]))
        ...
        def remTCPProxy(self, controller):
            ...
            present = True
            while(present):
                p = subprocess.Popen("iptables -t nat --check POSTROUTING -j MASQUERADE -p tcp --dport %s" % (controller.split(":")[1]), shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                out, error = p.communicate()
                if(not error):
                    os.system("iptables -t nat -D POSTROUTING -j MASQUERADE -p tcp --dport %s" % (controller.split(":")[1]))
                else:
                    present = False
